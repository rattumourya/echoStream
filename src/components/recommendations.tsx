...
export default function Recommendations() {
  ...
  const handleGetRecommendations = async () => {
    ...
    try {
      const result = await getPersonalizedRecommendations({
        listeningHistory: 'Daft Punk - Get Lucky, The Weeknd - Blinding Lights, Tame Impala - The Less I Know The Better, Queen - Bohemian Rhapsody',
        timeOfDay,
        dayOfWeek,
      });
  ...
  const recommendedSongs = songs.filter(song => recommendations.some(rec => song.title.toLowerCase().includes(rec.toLowerCase())));
      ...
      {!loading && recommendations.length === 0 && (
        <Card className="flex items-center justify-center p-16 text-center bg-muted/20 border-dashed">
            <div className="space-y-4">
                ...
                <p className="text-muted-foreground">Click the button to get personalized music recommendations from our AI.</p>
            </div>
        </Card>
      )}
      ...
      {!loading && recommendations.length > 0 && (
         <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
          {recommendedSongs.map((song) => (
            <div key={song.id} className="group relative cursor-pointer" onClick={() => playSong(song, recommendedSongs)}>
